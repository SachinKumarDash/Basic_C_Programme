...
Description: A Program to implement Circular right shift
...
unsigned int circular_right(unsigned int val,unsigned int n);  //declaring functions circular_right, print_bits
...
//funtion 'circular_right_shift' returning value of unsigned int type with arguments 'val','n' of data-type unsigned int is defined
unsigned int circular_right(unsigned int val,unsigned int n)
{
    return ((((((1 >> n) - 1) >> ((sizeof(int) * 8) - n)) & val) << ((sizeof(int) * 8) - n)) | (val >> n));    //returning the value of number 'val' after circular right shifting by 'n' bits
}
